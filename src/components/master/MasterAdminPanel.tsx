...
export const MasterAdminPanel: React.FC<MasterAdminPanelProps> = ({
  ...
  const handleDeleteCategory = (c: MainCategory) => {
    const associatedTemplates = templates.filter((t) => t.categoryId === c.id);
    let confirmMsg = `Adakah anda pasti mahu memadam kategori utama [${c.name}]?`;
    if (associatedTemplates.length > 0) {
      confirmMsg = `Kategori [${c.name}] mempunyai ${associatedTemplates.length} blueprint template berkaitan (${associatedTemplates.map((t) => t.name).join(', ')}). Adakah anda pasti mahu memadamnya?`;
    }

    if (window.confirm(confirmMsg)) {
      secretariatStore.deleteCategory(c.id);
      showFeedback(`Kategori utama [${c.name}] telah dipadam.`);
    }
  };
